...
pub trait GraphicsPipeline {
    ...
    fn get_global_wgpu_state() -> Result<Arc<State>, ErrorCode> {
        match global::get_global_wgpu_state() {
            Ok(state) => Ok(state),
            Err(err) => {
                error!("Failed to get the global wgpu state when creating a graphics pipeline: {:?}", err);
    ...
    fn from_multiple_shader_paths(
        ...
        Self::from_multiple_shader_modules(vertex_module, fragment_module)
    }

    fn from_single_shader_path(
        shader_path: &Path,
    ...
    fn from_multiple_shader_modules(
        ...
        let resources = match Self::init_resources(){
            ...
            Err(err) => {
                error!("Failed to init the resources when creating a graphics pipeline: {:?}", err);
        ...
        let bind_groups = match Self::init_bind_groups(&resources, &bind_groups_layouts){
            Ok(groups) => groups,
            Err(err) => {
                error!("Failed to init the bind groups when creating a graphics pipeline: {:?}", err);
                return Err(ErrorCode::Unknown);
            }
        };
        ...
        let render_pipeline = match Self::init_render_pipeline_from_multiple_modules(
            ...
            Ok(groups) => groups,
            Err(err) => {
                error!("Failed to init the render pipeline when creating a graphics pipeline: {:?}", err);
                return Err(ErrorCode::Unknown);
            }
        };
