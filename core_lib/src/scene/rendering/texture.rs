...
impl Texture {
    pub fn from_path(path: &Path, device: &wgpu::Device, queue: &wgpu::Queue, label: Option<&str>) -> Result<Self, ErrorCode> {
        let diffuse_bytes = match std::fs::read(path) {
            Ok(bytes) => bytes,
            Err(err) => {
                error!("Failed to read the texture from path `{:?}`: {:?}", path, err);
                return Err(ErrorCode::IO);
            }
        };

        match Self::from_bytes(&diffuse_bytes, device, queue, label){
            Ok(texture) => Ok(texture),
            Err(err) => {
                error!("Failed to create the texture from path `{:?}`: {:?}", path, err);
    ...
    pub fn from_image(image: &image::DynamicImage, device: &wgpu::Device, queue: &wgpu::Queue, label: Option<&str>) -> Result<Self, ErrorCode> {
